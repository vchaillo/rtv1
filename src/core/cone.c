...
/*   Updated: 2017/01/28 14:38:31 by valentin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float			hit_cone(t_cone *cone, t_ray *ray)
{
	...
	abc[0] = dot_product(ray->d, ray->d) - ((1 + pow_2(tangle)) * pow_2(tmp[0]));
	abc[1] = 2 * (dot_product(ray->d, dist) - ((1 + pow_2(tangle)) * tmp[0] * tmp[1]));
