...
/*   Updated: 2016/11/25 05:44:43 by vchaillo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				init_objects(t_scene *scene)
{
	...
	add_object(scene, new_object(SPHERE, new_sphere(-1, 1, 3, 1), new_color(LIGHT_GREEN), 1000));
	add_object(scene, new_object(SPHERE, new_sphere(4, 1, -4, 1), new_color(YELLOW), 100));
	add_object(scene, new_object(PLANE, new_plane(0, 1, 0, 0), new_color(DARK_GREY), 100));
	// add_object(scene, new_object(PLANE, new_plane(0, -1, 0, 6), new_color(DARK_GREY), 100));
	add_object(scene, new_object(PLANE, new_plane(1, 0, 0, 4), new_color(BROWN), 100));
